	...
	while (i < (*tokens)->size)
	{
		...
		else
		{
			...
			while (*new_words)
			{
				...
				new_token->str = ft_replace_char(*new_words, 1, ' ');
				if (new_token->str[0])
					deque_push_node_left(new_tokens, deque_node_init(new_token));
				new_words++;
			}
...
void	merge_words(t_deque **tokens)
{
	...
	while (i < (*tokens)->size)
	{
		...
		else
		{
			...
			new_token->type = TOK_WORD;
			new_str = ft_strdup("");
			while (i < (*tokens)->size && (travel->as_token->type == TOK_DQUOTE_STR
				|| travel->as_token->type == TOK_SQUOTE_STR || travel->as_token->type == TOK_WORD))
			{
				if (travel->as_token->type != TOK_WORD)
					new_str = ft_strjoin(new_str, ft_replace_char(travel->as_token->str, ' ', 1));
				else
